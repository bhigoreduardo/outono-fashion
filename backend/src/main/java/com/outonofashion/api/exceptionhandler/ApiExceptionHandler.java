...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	private ResponseEntity<Object> handleNegocioException(NegocioException ex, WebRequest request) {
		...
		ApiError apiError = createApiErrorBuilder(status, apiErrorType, message)
									.userMessage(message)
									.build();
	...
	// ENTIDADE_JA_CADASTRADA #3
	@ExceptionHandler(EntidadeJaCadastradaException.class)
	private ResponseEntity<Object> handleEntidadeJaCadastradaException(EntidadeJaCadastradaException ex,
			WebRequest request) {
		
		HttpStatus status = HttpStatus.CONFLICT;
		ApiErrorType apiErrorType = ApiErrorType.ENTIDADE_JA_CADASTRADA;
		String message = ex.getMessage();
	...
	private ResponseEntity<Object> handlePropertyBindingException(PropertyBindingException ex, HttpHeaders headers,
		...
		String detail = String.format("A propriedade '%s' foi ignorada ou é inexistente na classe '%s'.",
	...
	private ResponseEntity<Object> handleInvalidFormatException(
		...
		String path = joinPath(ex.getPath());
		String detail = String.format("A propriedade '%s' recebeu o valor '%s' inválido. O esperado"
				+" era um valor do tipo '%s'", path, ex.getValue(), ex.getTargetType().getSimpleName());
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(
		...
		List<ApiError.Field> apiFieldsError = bindingResult.getAllErrors()
									.stream()
									.map(objectError -> {
	...
	protected ResponseEntity<Object> handleValidacaoException(ValidacaoException ex, WebRequest request) {
		...
		List<ApiError.Field> apiFieldsError = bindingResult.getAllErrors().stream()
				.map(objectError -> {
					String name = objectError.getObjectName();
	...
	private ResponseEntity<Object> handleMethodArgumentTypeMismatchException(
		...
		String detail = String.format("O parâmetro da URL '%s' recebeu o valor '%s', que é um tipo inválido. Devendo ser "+
				"do tipo '%s'", ex.getName(), ex.getValue(), ex.getRequiredType().getSimpleName());
